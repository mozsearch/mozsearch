...
add_task(async function test_DiagramContextMenu_CallsTo() {
  ...
  is(query.value, "calls-to-sym:'_ZN14diagram_ignore2F2Ev' depth:4",
     "Query is set");
});
...
add_task(async function test_DiagramContextMenu_save_forget() {
  await TestUtils.resetFeatureGate("diagramming");
...

  {
    ...
    ok(!subsection, "Saved section is not shown");

    const buttons = listItem.querySelectorAll(".contextmenu-button");
    const save = [...buttons].find(x => x.textContent === "Save");
    ok(save, "Save button exists");

    TestUtils.click(save);
...

  {
    ...
    const subsection = listItem.querySelector(".contextmenu-subsection-title");
    ok(subsection, "Saved section is shown");
    is(subsection.textContent, "with diagram_ignore::F1", "Saved item is shown");

    const buttons = listItem.querySelectorAll(".contextmenu-button");
    const forget = [...buttons].find(x => x.textContent === "Forget");
    ok(forget, "Forget button exists");

    TestUtils.click(forget);
...
add_task(async function test_DiagramContextMenu_CallsBetweenTo() {
...

  {
    ...
    const buttons = listItem.querySelectorAll(".contextmenu-button");
    const button = [...buttons].findLast(x => x.textContent === "Calls to");
    ok(button, "Button exists");
...

  {
    ...
    const subsection = listItem.querySelector(".contextmenu-subsection-title");
    ok(subsection, "Saved section is still shown");
    is(subsection.textContent, "with diagram_ignore::F2", "Saved item is shown");
  }

  frame.contentWindow.localStorage.removeItem("diagram-saved");
});
...
add_task(async function test_DiagramContextMenu_CallsBetweenFrom() {
...

  {
    ...
    const buttons = listItem.querySelectorAll(".contextmenu-button");
    const button = [...buttons].findLast(x => x.textContent === "Calls from");
    ok(button, "Button exists");
...
add_task(async function test_DiagramContextMenu_CallsBetweenUndirected() {
...

  {
    ...
    const buttons = listItem.querySelectorAll(".contextmenu-button");
    const button = [...buttons].findLast(x => x.textContent === "Calls between");
    ok(button, "Button exists");
...
add_task(async function test_DiagramContextMenu_sync() {
  ...
  frame.contentWindow.localStorage.setItem("diagram-saved", "Func1");

  {
    ...
    const subsection = listItem.querySelector(".contextmenu-subsection-title");
    ok(subsection, "Saved section is shown");
    is(subsection.textContent, "with Func1", "Saved item is shown");
  }

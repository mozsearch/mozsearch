...
in
  clangStdenv.mkDerivation (finalAttrs: {
    ...
    checkPhase = ''
      ...
      test-index $(pwd)/tests/tests/checks
    ...
    installPhase = ''
      ...
      mkdir -p $out
      cp -RL index $out
      substituteInPlace $out/index/config.json --replace $(pwd)/index $out/index
      substituteInPlace $out/index/config.json --replace $(pwd) $src

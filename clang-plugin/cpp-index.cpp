...
public:
  ...
  void visitVariableDecl(ValueDecl *d) {
    ...
    if (VarDecl *vd = dyn_cast<VarDecl>(d)) {
      ...
      // TODO: See if there's a vd->getNameInfo(), as in other calls we make to declDef.
      declDef("variable", vd, def, vd->getLocation(), afterToken(vd->getLocation()));
    }
  }
...
protected:
                 ...
                 const std::vector<std::string>& args)
  {
    ...
    srcdir = abs_src;
    const char *env = getenv("MOZSEARCH_CXX_CLANG_OBJECT_FOLDER");
    if (env)
      output = env;
    else
      output = srcdir;
    ...
    const char* tmp = getenv("MOZSEARCH_CXX_CLANG_TEMP_FOLDER");
    if(tmp){
      tmpdir = tmp;
    }else
      tmpdir = output;

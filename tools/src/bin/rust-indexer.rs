...
fn scip_range_to_searchfox_location(range: &[i32]) -> Location {
    ...
    let line_end = if range.len() == 3 { line_start } else { range[2] as u32 + 1 };
...
fn analyze_using_scip(tree_info: &TreeInfo, scip_prefix: Option<&PathBuf>, scip_file: PathBuf) {
    ...
    for doc in &index.documents {
        ...
        let searchfox_path = match scip_prefix.and_then(|prefix| scip_file.strip_prefix(prefix).ok()) {
            Some(p) => {
        ...
        for occurrence in &doc.occurrences {
            ...
            write_line(&mut file, &get_target_data(&symbol.symbol, AnalysisKind::Use));
            ...
            for relationship in &symbol.relationships {
                if relationship.is_type_definition || relationship.is_implementation {
                    write_line(&mut file, &get_target_data(&relationship.symbol, AnalysisKind::Def));
                }
                write_line(&mut file, &get_target_data(&relationship.symbol, AnalysisKind::Use));
            }
        }

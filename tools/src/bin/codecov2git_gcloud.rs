...
struct Args {
    ...
    /// Only import revisions since (in RFC3339 format) (mainly for debugging).
    #[arg(long, value_parser=chrono::DateTime::parse_from_rfc3339)]
...
fn convert_hg_to_git(firefox_repo: &Path, hg_revisions: &[String]) -> anyhow::Result<Vec<String>> {
    ...
    let git_revisions = str::from_utf8(&output.stdout)?.lines();
    let valid_git_revisions = hg_revisions.into_iter().zip(git_revisions).filter_map(|(hg_rev, git_rev)| {
        if git_rev == "0000000000000000000000000000000000000000" {
            warn!("Ignoring Mercurial revision with no matching Git revision: {}. Ensure the underlying Firefox repo is up-to-date.", hg_rev);
            None
        } else {
            Some(git_rev)
        }
    ...
    Ok(valid_git_revisions.map(ToOwned::to_owned).collect())
}
...
fn extract_date_times(
...
) -> anyhow::Result<Vec<DateTime<FixedOffset>>> {
    ...
    let datetimes = git_revisions
        ...
        .map(|git_rev| -> anyhow::Result<_> {
            ...
            Ok(git_ops::git_time_to_chrono(commit.time()))
        })
        .collect::<Result<_, _>>()?;
    Ok(datetimes)
}

...
- `analysis`: Directory hierarchy that directly corresponds to the paths exposed
  ...
  indexing (ex: C++) occurs on the server and is inherently per-platform, with
  `process-gecko-analysis.sh` using the per-platform `process-tc-artifacts.sh`
  to process the per-platform data and then `collapse-generated-files.sh` and
  `merge-analyses.rs` to merge the per-platform data into merged analysis files.
...
- `gecko-blame`: git repository containing pre-computed per-file blame/annotate
  ...
  configuration.
- `gecko-dev`: Source git repoistory (using git-cinnabar).  The directory name
...
- `idl-files`: A list of all the '.idl' files in the tree produced by
  ...
  `mkindex.sh`.
- `ipdl-files`: A list of all the '.pidl' files in the tree produced by
  `find-repo-files.py` found and that the per-config `repo_files.py` didn't

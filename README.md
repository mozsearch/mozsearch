...
## Quick Start for Front-End Development (no Docker)

If you are working on the web UI (`static/`, `tools/templates/`,
`config_defaults/*.liquid`, `config_defaults/per-file-info.toml` or the Rust
code that renders source pages), you can skip the full setup below and use
`scripts/dev-server.js`. It serves pages from firefox-main using data from
searchfox.org, rendered locally with your changes.

You need Rust, `protoc`, Node 18+ and Python 3. Then:

```
cd tools && cargo build --release --bin output-file --bin crossref && cd ..
node scripts/dev-server.js
```

and open http://localhost:8000/firefox-main/source/.

- Source file pages are rendered locally by `output-file`, using the file
  contents from GitHub and the analysis data from searchfox.org at the
  currently indexed revision. The tools are rebuilt before each render, so
  changes show up on reload.
- Per-file info (test info, bugzilla components, CI results) and jump
  information are generated by running `crossref` on the files viewed so far,
  using the same Taskcluster artifacts as the indexer.
- `static/` is served from your checkout. Everything else (directory
  listings, search, blame, diffs) is proxied to searchfox.org.
- Per-file info can be overridden in
  `tools/target/dev-server/per-file-info.json` to test info boxes, e.g.
  `{"path/to/test.js": {"info": {"test": {"skip_if": "os == 'win'"}}}}`.

"Go to definition" only knows about symbols defined in files you have viewed,
and indexing itself isn't exercised; use the full setup below for that.
